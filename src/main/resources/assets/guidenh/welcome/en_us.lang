# Welcome to GuideNH

Welcome, <PlayerName />.

You are reading this page through <ItemImage id="guidenh:guide" /> **GuideNH**, an in-game guide framework designed for rich, data-driven documentation.

GuideNH is more than a plain book. It can combine Markdown text, item links, recipes, images, keybind hints, tooltips, and interactive scene previews into a single guide interface.

## Getting Around

Use the **navigation panel** on the left to browse guide pages.

Use the buttons in the top-right corner to search, return home, move backward or forward, and close the guide.

When a page is linked to an item, you can hover that item in your inventory and press <KeyBind id="key.guidenh.open_guide" /> to jump directly to its guide entry.

<Tooltip text="Some pages may include item links, recipes, structures, diagrams, or interactive previews.">
  Hover here for a quick tip.
</Tooltip>

## What You May Find Here

| Section | Purpose |
| --- | --- |
| <ItemImage id="minecraft:book" /> Articles | Explanations, tutorials, and reference pages |
| <ItemImage id="minecraft:compass" /> Navigation | Page trees, categories, and related topics |
| <ItemImage id="minecraft:crafting_table" /> Recipes | Crafting, processing, and usage information |
| <ItemImage id="minecraft:stonebrick" /> Structures | Multiblock previews and build guidance |

## Interactive Content

Some pages may contain interactive previews. These can help explain machines, structures, layouts, or special mechanics without requiring you to leave the game.

You may be able to:

- rotate and zoom the scene
- inspect individual blocks
- switch layers or variants
- compare different structure states
- follow annotations placed directly in the preview

<BlockImage id="minecraft:stonebrick" />

## For Readers

Start with the navigation panel, then search for the item, machine, or concept you want to understand.

If you are not sure where to begin, try opening the recommended pages on the home screen once they become available.

<Color color="#ffcc66">Tip:</Color> GuideNH works best when pages are written as small, connected entries instead of one very long document.

## For Authors

GuideNH pages are written as Markdown files with frontmatter metadata.

Authors can use GuideNH-specific tags such as <ItemImage id="minecraft:paper" />, <KeyBind id="key.attack" />, tooltips, item references, recipe previews, and scene components to make documentation easier to read in-game.

The goal is not only to explain content, but to make complex information easier to explore.

## Enjoy the Guide

GuideNH is here to make in-game documentation clearer, faster, and more useful.

Use it as a map, a reference book, and a workspace for understanding complex systems.
