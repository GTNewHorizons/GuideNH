 ...
 */
public sealed interface GuideRenderPrimitive
    permits
        GuideRenderPrimitive.PushTransform,
        GuideRenderPrimitive.PopTransform,
        GuideRenderPrimitive.PushScissor,
        GuideRenderPrimitive.PopScissor,
        GuideRenderPrimitive.SetBlendMode,
        GuideRenderPrimitive.SetColor,
        GuideRenderPrimitive.FillRect,
        GuideRenderPrimitive.GradientFill,
        GuideRenderPrimitive.DrawBorder,
        GuideRenderPrimitive.BlitTexture,
        GuideRenderPrimitive.DrawGlyphRun,
        GuideRenderPrimitive.DrawLine,
        GuideRenderPrimitive.DrawTriangle,
        GuideRenderPrimitive.DrawCircle,
        GuideRenderPrimitive.DrawCircleOutline,
        GuideRenderPrimitive.DrawPolygon,
        GuideRenderPrimitive.RenderItem,
        GuideRenderPrimitive.DrawText,
    ...
    record GradientFill(int x, int y, int w, int h, int argbTop, int argbBottom)
        implements GuideRenderPrimitive {}
    /** Border (4 sides, single color). */
    record DrawBorder(int x, int y, int w, int h, int top, int left, int bottom, int right,
                      int argb) implements GuideRenderPrimitive {}
    /** Textured quad with UV coordinates. */
    record BlitTexture(int texId, int x, int y, int w, int h,
                       float u, float v, float u2, float v2)
        implements GuideRenderPrimitive {}
    /** Glyph run from Rust cosmic-text shaping. Glyph positions are absolute document coords. */
    record DrawGlyphRun(int atlasId, List<PlacedGlyph> glyphs)
        implements GuideRenderPrimitive {}
    /** Line (thick or thin). */
    record DrawLine(float x1, float y1, float x2, float y2,
                    float thickness, int argb) implements GuideRenderPrimitive {}
    /** Filled triangle. */
    record DrawTriangle(float x1, float y1, float x2, float y2,
                        float x3, float y3, int argb) implements GuideRenderPrimitive {}
    /** Circle (filled or outline). */
    record DrawCircle(float cx, float cy, float radius, int argb,
                      boolean filled) implements GuideRenderPrimitive {}
    /** Circle outline only. */
    record DrawCircleOutline(float cx, float cy, float radius,
                             float thickness, int argb) implements GuideRenderPrimitive {}
    /** Filled polygon. */
    ...
    record DrawText(String text, int x, int y, int argb, boolean shadow)
        implements GuideRenderPrimitive {}
    /** 3D scene rendering. */
    record RenderScene3D(Object level, Object camera, List<?> annotations,
                         List<?> particles, List<?> weatherEffects,
                         float weatherAnimationTick, Object lightDarkMode,
