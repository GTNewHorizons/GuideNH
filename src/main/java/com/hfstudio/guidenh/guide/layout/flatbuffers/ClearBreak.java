...
/**
 * One in-paragraph clear break ({@code <br clear="..."/>
 * }). {@code rawOffset} is
 * the UTF-8 byte offset into TextData.text (the original text, U+FFFC included)
 * at which the break occurs; the Rust pusher maps it to its cleaned text and,
 * after the line covering that offset is laid out, drops subsequent lines to the
 * cleared floats' bottom edge. side: 1=left 2=right 3=both.
 */
...
public final class ClearBreak extends Table {
    ...
    public static void addRawOffset(FlatBufferBuilder builder, long rawOffset) {
        builder.addInt(0, (int) rawOffset, 0);
    }
