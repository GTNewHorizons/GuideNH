 ...
 *
 * <p>Adding a new shape is as simple as adding a new enum constant.
 */
public enum NodeShapeDefinition {
    ...
    public MermaidNodeShape shape() { return shape; }
    ...
    public static MatchResult match(String text) {
        ...
        for (NodeShapeDefinition def : values()) {
            if (text.startsWith(def.open) && text.endsWith(def.close)) {
                String inner = text.substring(def.open.length(), text.length() - def.close.length()).trim();
