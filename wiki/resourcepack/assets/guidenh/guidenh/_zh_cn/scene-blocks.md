...
这个游戏场景没有声明 `<BlockStats>`，但因为场景包含方块，方块统计切换按钮仍然可用。打开后会显示默认内部列表。如果没有声明 `maxWidth` 和 `maxHeight`，统计框默认限制为固定的 224 × 96 像素，当场景比这更大时会扩展到场景宽高的 40%，但至少会扩展到能完整显示 1 种方块，不会为了 1 个条目强行出现滚动条。
...
| `maxWidth` | 224 像素；场景更宽时为场景宽度的 40% | 统计框最大宽度，超出后出现水平滚动条。 |
| `maxHeight` | 96 像素；场景更高时为场景高度的 40% | 统计框最大高度，超出后出现垂直滚动条。 |
