...
The Region Wand uses one client-side global selection. Hold any item and run `/guidenhc bind` to bind its item id and metadata; NBT is ignored. Run `/guidenhc unbind` while holding the same item id and metadata to remove the binding. A bound item shows its Region Wand controls in the tooltip. Left click sets Pos1, right click sets Pos2, and both clicks can target air at the cursor reach endpoint. Sneak + left click clears the current selection; sneak + right click exports with the current mode. You can also use `/guidenhc pos1 <x> <y> <z>`, `/guidenhc pos2 <x> <y> <z>`, and `/guidenhc clearselection`; `~` coordinates are relative to the player. `/guidenhc exportstructure [--mode snbt|snbt_e|blocks|blocks_e]` exports the current selection, or accepts an explicit `<x> <y> <z> <sizeX> <sizeY> <sizeZ>`. The selection and bindings are client-local, so they work when GuideNH is not installed on the server.
