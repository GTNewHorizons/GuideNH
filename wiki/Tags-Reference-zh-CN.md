...
| `<FloatingImage>` | 支持浮动或真正行内放置的裁剪图片块 | `src`, `x`, `y`, `width` / `w`, `height` / `h`, `scaleX`, `scaleY`, `wrap`, `align`, `title` |
...
- `x` 和 `y` 为可选裁剪偏移（缺省 `0`）；`width` / `w` 与 `height` / `h` 至少提供一维，只给一维时另一维按图片自然宽高比推断
