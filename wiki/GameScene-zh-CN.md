...
列表会受 `maxWidth` 和 `maxHeight` 限制；如果省略，它们默认为固定的 `224` × `96` 像素，当场景比这更大时会扩展到场景宽高的 40%。内容过宽或过高时会出现可拖拽滚动条；鼠标在统计框上时滚轮会滚动列表，按住 Shift 可以横向滚动。
...
| `maxWidth` | 否 | `224` 像素；场景更宽时为场景宽度的 40% | 统计框最大宽度，超出后出现水平滚动条 |
| `maxHeight` | 否 | `96` 像素；场景更高时为场景高度的 40% | 统计框最大高度，超出后出现垂直滚动条 |
