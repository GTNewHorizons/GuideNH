...
## PF30: Fixed-viewport blocks decouple OUTER height from internal wrapping — measure the internal content, not the block

- **Incident** (2026-08-01, R6-2): after fixing code blocks to NoWrap, the executor measured the three scroll-container code blocks' OUTER LytCodeBlock heights (123/163/103) — byte-identical to pre-fix — and concluded "the fix had no effect." A qwen8-night probe proved the fix DID work: those code blocks have fixture-pinned `height=80/120/60` viewports, so outer height = toolbar(25)+fixed viewport+padding, decoupled from internal wrapping. NoWrap changed the INTERNAL content (python block, 106px viewport, 150+ char lines: h=50 single-line vs ≫50 if wrapped) which the viewport clips and which does NOT propagate to outer block height. The "no effect" conclusion was a wrong-metric false alarm.
- **Lesson**: when verifying a wrap/nowrap fix on content inside a FIXED-height viewport/scroll container, measure the INTERNAL content node height (or line count), not the outer container height — the outer height is pinned by the viewport and is insensitive to wrapping. More generally: before concluding "fix had no effect," confirm the metric actually couples to the changed behavior. (Sibling of PF29: wrong metric → wrong conclusion.)
- **Reference**: ISSUES.md §I; PF29.

## PF31: Run the ratchet EVERY screening round — mechanical assertions catch regressions VLM misses

- **Incident** (2026-08-01, R7/R8): two layout regressions — display LaTeX centering (R7-1, x=400→5) and large mindmap height (R8, 320→438) — survived R5/R6/R7 VLM screening undetected. Both were caught IMMEDIATELY when the ratchet (assert_bounds.py) was finally run: the `centered tol=3` assertion (display) and `max_height le 400` assertion (mindmap) failed. The ratchet existed and was documented as Stage 6, but was never executed during the screening rounds (only geo + VLM were run), so its assertions never fired.
