...
### X.6 User six-report fix wave (2026-08-05, ③/④ FIXED, ①/② pending)

- **③ Inline icon post-kerning gap — FIXED (PAD 2→0, LytItemImage.java:46)**. Root: INLINE_OPTICAL_PAD=2 (fa8d262c) padded both sides of every tight-advanced icon cell; post-kerning gap = right PAD + block 3D inset. Fix: PAD 2→0 (shared constant across computeContentSize:198 / computeLayout:256 / renderX:340), computeLayout no-label inline branch aligned to tightW (was 16px vs tight 20px hidden inconsistency). Verified: Requires-area blocks leading {0,3}→{0,1}, trailingCell 3→{0,1}, cell 16; full-page aggregate keeps trailing mode 3 for metrics-less fallback icons (coverage boundary, see below). Reviewer ACCEPT (no anti-pattern hits).
  - **Boundary (documented, not fixable in GuideNH)**: GT metatool etc. with custom IItemRenderer return null getIcon / getIconIndex on all paths → no pixel source → fallback 16px cell, tight advance impossible. tool_revamp A/B byte-identical pre/post (coder triple-evidence). Residual ~1.5 logical px gap on tool rows (user 220636 screenshot); PAD removal did not regress anything.
  - **Independent finding**: nac idx=113 (y=6703) icon renders iconW=1 (near-missing) — same family as AG4's y=471/539 missing rows; specific items fail headless 3D render. Recorded, not chased.
  - **Process incident**: coder used `git checkout -- IconMetrics.java` to revert its own experimental edit (disclosed; executor verified zero residue, LytItemImage changes intact, no collateral). Red-line violation logged; no recurrence tolerance.
- **④ WARNING ⚠ fragment render — FIXED (P5-C, parley_text.rs:88-90)**. Root: parley classifies U+26A0/U+2622 as emoji/pictograph (parley_data baked ICU) → GenericFamily::Emoji queried in main families (parley-0.11.0 shape/mod.rs:580-591) → fontique DWrite resolves to system seguiemj.ttf (COLR composite, backend/dwrite.rs:31-41) → 22px collapse into 22×10 fragments; P5-B's seguisym sat in fallback_families (Hani key) and was bypassed entirely. Fix: append_generic_families(GenericFamily::Emoji, seguisym ids) — fontique yields ours before system (collection/mod.rs:350-362), seguisym now wins emoji queries. Verified: ⚠→seguisym gid 1549, ☢→gid 1425 (parley_probe + render log); 225 gold px vs 26 fragment; vlm ask visual: "complete triangle+exclamation"; NOTE/TIP/IMPORTANT regression clean; DLL deployed both natives (SHA256 match); seguiemj usage 0. Note: ⚠ renders hollow-outline triangle — swash/zeno rasterization of seguisym gid 1549 (inherent, FreeType renders solid); NOT a regression.
- **Infra: ask subcommand live** (tools/visual-inspection/screen.py): --files + --prompt, whole-image, free-answer + best-effort findings. First combat use: verified ④ WARNING glyph shape. vlm/geometric/report unchanged.
- **Carried**: ① H1 toolbar-title offset + ② navbar double-pass ghosting (live-path, static-trace next); GT tool-row residual gap (~1.5px, boundary); idx113-class missing icons; P5-C ⚠ hollow outline (swash inherent); tool_revamp A/B identical (GT boundary).
  - **① Page-title toolbar offset — FIXED (GuideScreen.java:3983-3985)**. Root (T6): drawPageTitle titleX = panelX + PANEL_PADDING (59cd3e2c2, predates S1 navbar integration) — document path carries navReservedWidth (:958/:979-981/:4506), title did not → title rendered inside navbar column (pixel baseline on user 220403: title x[28,172]px co-located with navbar rows x[179,201] same y-band, logical origin ≈14 in navbar column [0,150]). Fix: titleX += getNavigationReservedWidth(), availableW -= navReservedWidth (right edge unchanged). Reviewer ACCEPT (no anti-patterns). Live-only defect (headless never draws toolbar) — live confirmation pending user in-game look. VLM ask misread this overlap (reported "no overlap, content area") — pixel forensics overrode (PF26 re-confirmed).
  - **② Navbar ghosting in scrolled state — FIXED (GuideNavBar.java + RenderPageService.java + build.gradle.kts navscroll forward)**. Root (T7): double-pass hypothesis REFUTED (executeNavText runs once per frame, GuideScreen.java:2873 sole call; no render mixins). Real root: regular-row loop y (getRowY :994, scrolls) vs sticky-row loop y (computeStickyStack :586-617, pinned near body top) — two independent y sources, no screen-space avoidance; scrolled rows pass through sticky rows' fixed bands → same-position double draw; body scissor (:461) → half-line ghosting. Fix: overlapsStickyRow band-intersection skip in BOTH mirror loops (live :481-487, headless :1192-1198), stickyRowY Math.max(bodyY,…) lower bound (:622, closes X.5 z-order item), headless scrollY injection hook (GuideNavBar.setScrollY :994-1005 + RenderPageService -Dguidenh.renderpage.navscroll :539-556, default byte-identical). Verified: injected scrollY 0/30/60/120/240 five frames ALL OK (executor end-to-end rerun with navscroll=30: injected log + pixel ALL OK); scrollY=0 regression matches AG1/AG2 baseline; sticky never enters title band. Reviewer ACCEPT (independent Node re-analysis of 5 frames, no anti-patterns).
  - **Wave-3 existence confirmation (AG6, 2026-08-05)**: R4-12-left → CLOSED (fixed in 98413454, 07-30, post-STUCK; fresh render shows full list content in left node, pixel+VLM evidence). T1 edge remnants (^^/:: + mermaid kind 4/5) → EXISTS but dormant (code paths: LytMermaidCanvas.java:614-618 4-arg GlyphRunData drops kind; LytItemImage.java:465-472 parses ^^/::, GuideText.java:123-125 never emits; corpus zero usage) → recorded, not fixed. code_blocks L113 → EXISTS (fixture, blocks.md:113 raw <>&"§ untouched since 6fc0a626) → fix queued. R3-13 mermaid edge labels → EXISTS confirmed (label right edge 1px from node border, label fills 90%+ of gap) → fix queued. F7b-2/3 scene slider labels → EXISTS (LytGuidebookScene.java:3344/:3357 Block Stats, :6909 load-state still VanillaRenderContext.drawString pixel font; F7b-1 pattern available) → fix queued.
  - **Wave-3 fixes (2026-08-05)**: code_blocks L113 → FIXED (blocks.md:113 HTML-entity escaped; render ok, parse error gone, heading content unchanged; product run/client_new/renders_r47/). R3-13 edge labels → FIXED (LytMermaidFlowchartCanvas.java EDGE_LABEL_GAP=1 + free-span clamp + wrap/clip for oversized labels + single-round center fix; three labels now 5/3·5/3·5/7px gaps vs 1px before; other 3 no-label graphs MD5-identical; product run/client_new/r313_final2/). F7b-2/3 scene text → FIXED (LytGuidebookScene.java hybrid migration: Block Stats name/count + load-state via GuideText.emitText, suppress flags mirror F7b-1; count text pixel-verified vector font via temporary config flip guienh.cfg sceneBlockStatsVisible=true then reverted — evidence source documented here; name/load-state code-path verified only, no fixture triggers; product run/client_new/screenshots_f7b23{,b}/). All three reviewer ACCEPT (run/ dir is gitignored so render products invisible to glob — they exist). R4-12-left CLOSED. T1 dormant recorded. Full-corpus regression pending.
  - **Full-corpus regression (2026-08-05, post wave-1..3)**: 266 pages (en_us renderable corpus; 418 = all-language md count) rendered 5 batches, 266/266 ok, zero glyph-atlas/OOM/black-GL pages; geometric 11 findings byte-identical to baseline (zero new); ratchet 55/55 PASS; VLM sample (35 pages) 136 findings adjudicated: ALL false-reads / pre-existing / environment noise, ZERO new real defects. Adjudication evidence: callouts icon-text spacing 12px (VLM "embedded" false); lists_tasks 🔵 vertical centers 1223.5 vs 1225.5 (VLM "misaligned" false, bbox out-of-range); flowchart edge labels centered 5/3 gaps (VLM "vertical stack" false — wrap of oversized labels misread); inline-marks 0 ink (false); wrap-long-labels long string wraps 4 lines, node height adapts (VLM "overflow" false, ask re-verified); subgraphs/charts/scenes findings = pre-existing families (R3-12/R4-25); callout-combos = X.5 known quirk; changelog ItemLink red errors = pre-existing corpus data defects. Environment note: regression batch b1 flowchart PNG (141551) carries 4 grey 15x18px blocks absent in pre-fix/post-fix/current-code renders — root: concurrent gradle build race during regression batch (jar timestamp 13:27, PF9); current-code re-render (145441) clean; not a code defect.

## X.7 User re-verification (2026-08-05, live in-game) — 2 NOT-FIXED + 1 SUSPECTED-HACK (marked, no action)

- **① Page-title offset — REOPENED (fix 3983-3985 NOT effective in live)**. User in-game verification: title still misplaced after the titleX += navReservedWidth fix. T6 root (titleX lacks navbar offset) was code-confirmed and pixel-baselined, yet live behavior unchanged → the fix's premise is incomplete or a second mechanism overrides toolbar title position in live (e.g., drawPageTitle not the only/live title draw path, or getNavigationReservedWidth resolves 0 in the user's layout state, or toolbar title y-collision remains the visible defect). Requires re-diagnosis on the LIVE path (was headless-invisible). Marked NOT-FIXED, no action per user directive.
- **③ ItemLink kerning — REOPENED (PAD fix NOT effective for ItemLink)**. User: ItemLink kerning still wrong in live. PAD 2→0 verified on ItemImage tight-advance rows (nac Requires blocks, tool rows bounded by GT null-icon); ItemLink (link-wrapped item tags, incl. label mode) has its own kerning/advance path not covered by the PAD change. Requires path-specific measurement (ItemLink advance vs ItemImage; live vs headless). Marked NOT-FIXED, no action.
- **Latex display block — SUSPECTED HACK (marked for investigation, no action)**. User suspects display-formula blocks are vertically stretched (scaleY?) — looks like a hack forced through acceptance (ratchet/assertions). R7-1 fix (12b0b826) restored a NARROW full-width rule for LytLatexDisplayBlock (width-side, for centering) — width-side change should not stretch Y; need to check LatexDisplay render path for any vertical scale/height override and when it was introduced. Marked SUSPECTED, no action.
  - **F4 glyph atlas full — BLEED-STOP DEPLOYED (2026-08-05, parley_text.rs:817-822)**: live reproduction captured (fml-client-latest.log 15:06:53-15:07:16: 441 WARN / 33 keys / 966x1086~1402x1517 bitmaps; triggered by mermaid afterExternalLayout+computePrimitives; game closed after). Diagnosis (numerically closed): multiplier point = parley_text.rs:786 size = g.font_size × render_scale; 11 × fontScale(≈33) × renderScale(3-4) = 1089-1452ppem matches observed bitmaps exactly; candidate-2 (MermaidNodeRenderer zoom×fontScale) numerically excluded (≤110ppem); injection point of fontScale≈33 NOT statically locatable (all static paths value-bounded) → live tracing required. Bleed-stop: MAX_RASTER_SIZE=128 clamp + atlas key derived from CLAMPED size (flood structurally eliminated) + one-shot-per-key diagnostic (font_size/render_scale/raw_size/bitmap/clamped_to/glyph_id) as injection-tracing payload. Probe-verified on disk (f4_probe_out.txt: 1089ppem → exactly 1 WARN font_size=33 render_scale=33, dedup works, 27.5ppem baseline silent). Headless regression clean (mermaid 3 pages + text page: 0 clamp WARN / 0 atlas-full). Status: tourniquet live, primary fix = injection point, awaiting ONE live reproduction to capture the clamp diagnostic line → then root fix (fontScale/renderScale must never exceed bounded semantics).
  - **F4 glyph atlas full — ROOT CAUSE CLOSED + FIX DEPLOYED (2026-08-06)**. Runtime data (00:14 live session, [F4TRACE] instrumentation): ALL 186 oversize glyphs came from GuideText.shapeUncached with **fontScale=0.0** (texts: Blocks/Items/Tools/Shaped/Shapeless/Recipes/GuideNH — UI/panel labels), renderScale=2. Mechanism chain (closed): fontScale=0 → Java ShapeTextInput fontScale=0 → Rust scaled = font_size × font_scale = 0 → parley shapes at size 0 → OutGlyph.font_size = 0 → clamp condition `raw_size = g.font_size × render_scale > 128` is FALSE for 0 (0>128=false; NaN>128=false) → swash renders at size 0/NaN → outputs raw outline-coordinate bitmaps (1371x1516 etc.) → atlas flood. Rust stderr (eprintln) does NOT reach fml logs — earlier "clamp never fired" was a channel artifact, not absence. Fix (parley_text.rs:834-851): non-finite or ≤0 raw_size falls back to DEFAULT_RASTER_SIZE=22 (normal-size bitmap, key derived from fallback size); normal path byte-identical; clamp retained for finite positive oversize. Probe-verified on disk (f4_fix_probe_out.txt): font_size=0 → 16x16 bitmap + [F4TRACE] fallback=22; NaN → 16x16; normal 22ppem silent; 1089 → clamp 128 + WARN. Java stack-trace instrumentation added (GuideText shapeUncached, fontScale≤0.01 prints caller stack) to capture the fontScale=0 injection source on next natural use. DLL 6c9409cf deployed (jar-embedded verified); headless smoke clean. Status: fix live, root injection point pending stack-trace capture on next natural use.

## X.8 Mermaid fixture wave (2026-08-06, injection + interaction chain + 3 defect fixes)

- **Fixture expansion (9 new pages, visualtest/.../mermaid/)**: directions (LR/TB/RL/BT + graph synonym), shapes (all shape variants + Chinese labels), links (arrow/linkStyle/edge-label matrix), deep-subgraphs (5-level nesting), stress-large (100-node flowchart + 64-node mindmap), mindmap-variants (12-level chain + tidy-tree), rich-text (bold/italic/code/br/CJK/emoji/URL), node-content-tags (4 <Mermaid> w/h combos), mixed-multi (7 alternating diagrams). All rendered non-stub (LytCodeBlock count 0, diagram count == fixture segment count).
- **Headless injection (T1, LytMermaidCanvas + RenderPageService)**: -Dguidenh.renderpage.mermaidzoom (double, 0=none) + mermaidoffset ("x,y") mirroring navscroll pattern; HEADLESS branch (computePrimitives :398-422) honors injection, else historical fitZoom+centre — byte-identical without injection (md5-verified mindmap e8430e19 / flowchart 2f7a73bc). build.gradle.kts:79 forwards.
- **Interaction chain root (T1)**: LytMermaidCanvas.updateVisualState() (:238-244) had NO caller — wheel writes raw zoom (scroll :288-309), drag writes raw contentOffsetX/Y (dragTo :274-281), render reads visual (getActiveZoom/getVisualOffsetX/Y) → permanently detached → zoom/drag "don't work". Fixed: per-frame drive at computePrimitives entry (:366, LytCodeBlock:260 pattern).
- **Font-scale root (T1)**: MermaidNodeRenderer.getOrScaleStyle (:91-94) cache keyed by base style only, lastScaledStyleZoom (:76) dead — zoom change kept stale fontScale. Fixed: Float.compare(zoom, lastScaledStyleZoom) != 0 → clear cache (:437-449). Verified via injected renders: text height scales 16→32px at zoom=2 (2.000x, box ratio constant).
- **Injection-batch visual verification (11 combos)**: zoom 0.5/1.5/2 + offset(100,50) on mindmap/flowchart, z0.5/z2 on mixed-multi, z2 on stress-large — ALL OK: content and text scale synchronously, no overlaps/breakage; clipping at zoom>1/offset = expected. Screener + executor adjudication.
- **Baseline 15-page screening → 3 real defects**:
  - **A edge labels vertical-stack (high)**: label wrap budget took node-gap (span=20px, nodeSpacing) as label width → any label wider than 14px degraded to per-char vertical stack. Root: LytMermaidFlowchartCanvas.java:796 budgetPx = span - 2*EDGE_LABEL_GAP - 2*pad. Fix: :831-866 budget = edge path total length (screen space); R3-13 preserved (gap-budget re-wrap keeps full text, clip only when single line exceeds gap). Verified: Critical → Cri/tic/al 3 lines (was 8 single-char), Fallback 2 lines, Yes/No/Plain horizontal single-line, probe hitsFrom/hitsTo all false.
  - **B shape-node text wrap/overflow (medium)**: minNodeRect zero-margin + render-time contentBounds integer truncation under zoom (SubprocessShape FRAME_WIDTH unscaled, CloudShape insSide truncated) → scaled content width < scaled text width by 0.5-6px → wrap + overflow ('级' below hexagon edge). Fix: NODE_SIZE_ROUNDING_MARGIN=2 (:52-58/:281-282), contentBounds(zoom) signature across 14 shapes (ShapeRenderer.java:10-22), SubprocessShape frame=round(8*zoom) (:24-32), Cloud/Circle/DoubleCircle ceil(r*√2) (:98-108 etc.). Verified: shapes.md 3 cases single-line no overflow.
  - **C 100-node canvas right-edge squeeze (high)**: precomputeLayout preferredWidth = diagramWidth unconditionally (1695 = layout width, 2x page) → page-width clamp dead code in Rust path (LytMermaidFlowchartCanvas.java:180 never executed) → canvas overflow → height-driven fitZoom centres in 1695-wide canvas → visible region = right half. Fix: preferredWidth = min(diagramWidth, max(1,safeWidth)) (:333-339) + precomputedLayoutWidth sync. Verified: stress-large canvas 1709→494 (bounds JSON), layout centred, no left blank.
- **Review**: reviewer ACCEPT (anti-patterns none; 480 clamp deviation and short-edge geometry deviation both technically valid). Independent re-render 10 pages: mindmap/node-content-tags IDENTICAL (md5), links x3 stable b8219869, diffs localized (C single-band, B shape segments; links/flowchart/subgraphs height +16-38px = downstream shift from B margin, not content drift). Coder self-report error: large diff % (claimed 19.267, actual 6.634 — data precision only). Post-fix re-screening (7 pages): OK except Chinese edge label — see residuals.
- **Residuals (registered, no action)**: (1) **CJK edge labels still per-char vertical** — Chinese 标签 (links.md) wraps per-codepoint (CJK word-break semantics: each char = own word), Latin labels fixed (Cri/tic/al). A-fix partial for CJK. (2) Short-edge horizontal labels physically impossible without ELK reserving label space (Critical 44px vs 20px gap) — full-horizontal requires edge-label-aware ELK layout, not implemented (regression-risk). (3) C clamp ceiling 480 = precompute hardcoded pageWidth (PreCompiler.java:227/250, MermaidScript.java:173, out of whitelist) vs real page 880 — 480-880 canvases (shapes 486) get 1.3% minify, acceptable. (4) SubprocessShape visual frame (emitPrimitives FRAME_WIDTH=8) not zoom-scaled vs contentBounds frame=round(8*zoom) — text stays inside visible frame, no visual defect. (5) rich-text math expr wrap (low conf 0.6) observation. (6) node-content-tags width constrained vs height exact (200/400/900/1200 declared → 192/235/440/404 actual) — asymmetric, known behavior, unresolved.
- **Pending live verification (user)**: wheel-zoom + drag now drive render via updateVisualState (interaction chain fixed — live confirm); live font-scale sync during zoom (cache invalidation — live confirm).
- **Worktree**: mermaid fixes (LytMermaidCanvas/LytMermaidFlowchartCanvas) + 9 fixtures + RenderPageService/build.gradle.kts uncommitted — awaiting user commit direction (mermaid 组等指令).

### X.9 Glyph atlas paging (2026-08-06, architecture fix: mature pattern 1)

- **Bug (live)**: over-zooming mermaid dropped characters. fml-client-latest.log 20:38:56-20:39:18: **181,175** `glyph atlas full, dropping glyph` WARNs; glyphs uniformly ~60px (zoom 2.7x → fontScale → large rasterization); single 2048² atlas holds ~1000 such glyphs → full → new glyphs dropped (characters lost); failures not cached → per-frame retry flood. Not F4-class (no F4TRACE/OVERSIZE/bleed-stop).
- **Architecture verdict (user-led)**: layout-level zoom (fontScale follows zoom, browser Ctrl+zoom pattern) is sound; the defect was cache management — GuideGlyphAtlas was single-texture fixed-capacity, drop-new-on-full, no eviction (half of mature pattern 1). User confirmed pattern-1 completion: paging + LRU + failure cache + zoom cap.
- **Fix (T3, GuideGlyphAtlas.java rewrite)**: MAX_PAGES=4 × 2048² (16MB/page, 64MB cap, :62); upload→GlyphSlot(pageId,uv)/null (:110); lookup refreshes page accessStamp (:163); resolvePackingPage (:140) opens new page, evictPage (:275) evicts oldest page (glDeleteTextures + clear page cache + reset cursor); **C-3 red line held: no capacity-triggered full clear anywhere** (clear() remains caller-initiated API, zero new callers — grep); failure set (:336 markFailed): structurally-unplaceable keys → silent null + one WARN per key lifetime (:80), cleared on eviction (:157), evicted keys NOT in failure set.
- **Render path**: DrawGlyphRun atlasId removed (GuideRenderPrimitive.java:80, both emitters LytParagraph/GuideText cleaned); GuideRenderEngine.emitGlyphQuads (:601-617) resolves page per key, flush+rebind on page change; fixed two regressions found en route: tessColor wiped by Tessellator.startDrawing (hasColor reset — color moved after beginTexturedQuads, :612-616), literal MIN clamp would raise fitZoom (fitZoom≈0.44 < MIN_ZOOM=0.5 for stress-large — fit path exempted, upper-cap only, byte-identical preserved).
- **Zoom cap**: MAX_ZOOM 2.5→5.0 (LytMermaidCanvas.java:49), clamp sunk into getActiveZoom (:224-233, covers headless injection :431-437 — z8.0 renders byte-identical to z5.0, md5 53427cb6). Rust untouched (128ppem hard cap already in place).
- **New unit test**: GuideGlyphAtlasPagingTest 8 cases (cross-page packing, LRU eviction victim, evicted-key miss/re-pack, structural failure silent+WARN-once, failure-set cleared on eviction, headless clear/delete, row-layout UV-identical to legacy). All green.
- **Verification**: reviewer ACCEPT (no anti-patterns, C-3 red line verified, 8 tests statically traced). Independent regression (T4): no-injection 10/10 byte-identical vs verify/ baseline (mindmap e8430e19, links b8219869 etc.); stress-glyphs.md (6763 unique hanzi, 6985 unique chars — new fixture) at zoom 2.5/5.0/8.0 all render OK, zero drops, zero floods, log gates all 0. 5 pages N/A vs verify/ (baseline dir lacks them — logical equivalence covered by 10/10 + unit tests + reviewer static; registered).
- **Residuals**: live multi-frame accumulation (scroll/zoom across pages) may evict cold pages — recovery: per-frame labels ≤1 frame, document glyphs at next layout (designed, C-3-scaled granularity); LRU doesn't exclude current write page (harmless — reviewer note); clear() doesn't reset failureWarned (consistent). Live verification pending user natural use.
- **Fixture**: stress/stress-glyphs.md — GB2312 full 6763 hanzi + 95 ASCII + symbols + 3 long CJK paragraphs + mermaid 22-node flowchart + 16-branch mindmap; rendered non-stub (0 atlas full at zoom 1 — exactly at capacity edge; the paging regression carrier).
- **Worktree**: paging changes (GuideGlyphAtlas/GuideRenderEngine/GuideRenderPrimitive/GuideText/LytParagraph/LytMermaidCanvas + test) uncommitted alongside mermaid wave — awaiting user commit direction.

### X.10 Title alignment + atlas capacity/jitter wave (2026-08-07)

- **Toolbar title x/y defects (live, re-report of X.7-①) — FIXED + headless channel**. Dual root: (A) "floats on top" = pageTitle used HEADING1 style (fontScale 1.5 → line height 26) in the 16px toolbar (GuideScreen TOOLBAR_H=16) → titleY clamp → 10px overflow painting over content below (scrollOffsetY=0 → fixed overlay feel); (B) titleX aligned to baseContentX (:3985) not contentX (:963 narrowReadingInset). Fix: TOOLBAR_TITLE_STYLE fontScale 0.8 (mirrors GuideNavBar TITLE_FONT_SCALE, line 14 < 16) at GuideScreen.java:245-250/:541; titleX = panelX+PANEL_PADDING+navReservedWidth+narrowReadingInset (:4003-4005, same formula as contentX); defensive viewport Math.min (:4021). Headless channel added: -Dguidenh.renderpage.title=true renders drawPageTitle-equivalent path (RenderPageService.java:348-363/:686-781, default strict no-op — byte-identical verified md5 e8430e19), build.gradle.kts:79 forwards. Verified: title bbox x 170.0-170.5 (contentX 170 → 0.5px), bottom 15.5 < 16 (no overflow), 3 pages; reviewer double-pass ACCEPT (P1 headless channel missing → REJECT → re-fixed + re-ACCEPT; P2/P3 comment truthfulness fixed). Executor adjudication: x-alignment acceptance = structural (formula identical to live contentX) + live pending user — headless geometry has no navbar offset (document at x=0, CONTENT_PAD=14), pixel-parity vs first block is structurally N/A.
- **Atlas capacity overflow at render_scale=4 (live stress-glyphs char loss) — 8-page capacity + eviction log**. Diagnosis: live guiScale=auto 1080p → render_scale=4 → 44ppem glyphs (11×1.0×4) → 4-page capacity 8,100 vs stress-glyphs 6,963+session residue → silent whole-page LRU eviction (evictPage had ZERO logging) → stored-run glyphs permanently missing (no re-upload path until layout rebuild; layout cached by width/renderScale). Headless blind spot confirmed: batch renders at render_scale=1 (11ppem, capacity 115,600 — never triggers; the 10/10 md5 regression only covered s=1). Fix: MAX_PAGES 4→8 (GuideGlyphAtlas.java:61-67, 128MB, 2x margin); evictPage WARN with 1s throttled aggregation (:295-333) — reviewer REJECT caught sentinel bug (Long.MIN_VALUE overflow → log dead) → fixed (0L initializer, :95); guiscale headless injection (-Dguidenh.renderpage.guiscale, RenderPageService:295-372/:785-808) — **ineffective headless** (ScaledResolution caps effective scaleFactor; gs4 render byte-identical to no-inject md5 1ac40ee4) — s=4 pressure not reproducible headless, capacity fix stands on arithmetic (16,200 ≥ 6,963+residue), live verification pending user.
- **Mermaid zoom jitter — zoom quantization (d①)**. Root: easing emits a fresh f32 zoom per frame → fontScale fresh per frame → GuideText ShapeKey (includes fontScale, GuideText.java:52) all-miss → per-frame Rust re-shape + re-rasterize + per-glyph upload (30-150 keys/frame, 8-40MB/s) → frame-rate collapse 5-50fps; atlas-full superimposed page eviction flashing (13.5 frames/page @s=4). Fix: getActiveZoom quantizes interactive/injection paths to nearest 1.1^n (1.1^round(log(v)/log(1.1)), clamp [0.5,5.0], LytMermaidCanvas.java:224-262; no-inject fit path exempt — md5 byte-identical e8430e19 preserved); quantize log "requested/quantized" (:467-471). Verified: 2.0→1.9487174, 5.0→5.0 (clamped), stress-glyphs@z5 byte-identical to pre-quantize baseline (53427cb6); unit tests 8/8 dynamic (FULL_PAGES). Reviewer: 4/5 PASS, REJECT→sentinel fixed→closed. Jitter elimination (cache hits during animation) — live pending user.
- **Residuals**: s=4 live verification pending (capacity + no-char-loss); zoom animation smoothness live pending; guiscale injection only affects live-ish paths (headless capped); quantizeZoom MIN-end semantics 0.5→0.513 (documented, within [0.5,5.0]); PagingTest method name "FourPages" stale (comment fixed, name left). Font-size question answered (design: nav 0.70/0.80 vs body 1.0 — intentional per GuideNavBar.java:170-176; body 17px baseline ≈ 1.9x MC native UI — maintained).
- **Worktree**: title fix + capacity/jitter wave + all prior mermaid/atlas waves uncommitted (GuideScreen/RenderPageService/build.gradle.kts/LytMermaidCanvas/GuideGlyphAtlas/LytParagraph/GuideRenderEngine/GuideRenderPrimitive/GuideText/shapes/*/DefaultStyles) — awaiting user commit direction.

### X.11 Typesetting red-line findings (2026-08-07, render-mechanism audit — USER-DECLARED RED LINES, no action yet)

User-declared red lines (typesetting incorrectness — fix REQUIRED, not acceptable):
- **LaTeX display-formula sizing — RED LINE (typesetting incorrect)**. Display block scales the reference "x" to body LINE HEIGHT (17px): formulaDisplayW/H = ceil((size+pad) × lineHeight × userScale / (refH+pad)) (LytLatexDisplayBlock.java:165-166/184-185) — independent calibration. Inline formulas instead use body x-height × 1.2 (LytLatexBlock.java:257-298) — the two standards disagree, display renders ~1.55x larger than body. Mature convention (MathJax/KaTeX/LaTeX): display math at body font size, baseline-aligned. NOT a vertical-stretch hack (W/H share one ratio; no scaleY anywhere — X.7 stretch suspicion refuted), but the calibration choice itself is wrong. Fix direction: display calibration aligned to body/inline standard (x-height × 1.2 or body font size) + re-calibration regression. PENDING.
- **Italic kerning — RED LINE (typesetting incorrect)**. Italic = render-time synthetic shear only (GLYPH_SHEAR_K=0.25, GuideRenderEngine.java:38/:626-629 — vertex horizontal offset, pen/advance untouched). Kerning is computed in Rust parley shaping on the UPRIGHT glyphs (shaping input carries no slant: push_spans only Brush+FontWeight, parley_text.rs:508-524; no italic family passed) — shear never participates in kerning, slanted glyph top overhang beyond advance is uncompensated (line-start/end hang). Recorded as "inherent italic slant" at ISSUES.md:504 — that acceptance predates this red-line declaration; the slant-induced visual spacing error is typesetting-incorrect per user. Fix direction: pass slant into shaping (or compensate first/last glyph horizontal overhang at draw). PENDING.

Additional findings (registered, severity per user):
- **dotted/wavy decoration rasterization at 1x doc grid** (DecorationRasterizer: 8-phase sine wavy / 4px-dot dotted, integer doc-pixel columns, antialiased via subsampling) vs glyphs re-rasterized at render_scale — decorations do not resample with scale → steppy/mosaic look on zoom. Not a red line; fix direction (if scheduled): resample decorations at render_scale.
- **Mermaid zoom jitter residual — integer geometry** (X.10 quantization landed; remaining source): mermaid geometry is all-integer (baseX/baseY, scaled() Math.round, emitText Math.round, offset rounded) + cross-quantile fontScale jump + parley xi/yi truncation — easing exists (SmoothFloatState) but integer truncation + quantile jumps cancel it. Fix direction (if scheduled): float geometry + inter-quantile interpolation (tension with quantile caching).

### X.12 Title fine-tune (registered, AWAITING MULTIMODAL, no action)

- **Toolbar title position roughly correct now (left-edge placement at panelX+PANEL_PADDING), but typography not polished** — user: "排版仍然不美观，需要微调". Registered as issue; fine-tune direction (spacing/vertical centering/size nuance) to be decided by multimodal (q37p-screener) visual review. NO action until multimodal verdict.

### X.13 Red-line fixes closed (2026-08-08): LaTeX display sizing + italic kerning

- **LaTeX display sizing — FIXED (reviewer ACCEPT)**. display calibration merged into the inline standard: formulaDisplayW/H = ceil(size × xHeight × 1.2 × userScale / (refH − 4)) (LytLatexDisplayBlock.java:126-130 displayScaleFactor, W/H share one ratio — no stretch), legacyPaddingDiff removed (was padding to preserve pre-fix baseline), INLINE_PERCEPTUAL_FACTOR/LATEX_INSET_PX promoted package-private (LytLatexBlock.java:102/:115, inline logic untouched — anchor). Verified: display block heights 0.656-0.727 of pre-fix (1/1.37-1/1.52), cross-page same-formula display/inline ratio 0.93 (1px = STYLE_DISPLAY vs STYLE_TEXT icon diff, mature-convention consistent), inline heights byte-identical pre/post, latex ratchet 6/6 ok. Site export (GuideSiteHtmlCompiler ×20.0) kept — site display/inline already share one formula + no font-engine x-height available (registered).
- **Italic kerning — FIXED (reviewer ACCEPT)**. Synthetic-slant overhang compensation (mature synthetic-italic practice, equivalent to true-italic-face hmtx): SpanStyle.italic (parley_text.rs:242-246), measure passthrough (:174/:273), GlyphRun grouping per-span + italic_kerning_compensate (layout.rs:354-403, baseY = run max(q.y+q.h) same basis as engine GuideRenderEngine.java:593, x'_i = x_i + Σ_{j<i} K·(baseY−y_top_j), line-boundary reset), GLYPH_SHEAR_K=0.25 cross-referenced (:19-26), single-style paragraph fix (base_italic from base TextStyle, :341-345/:411 — was shear=false always). DLL rebuilt (md5 3004f893, dual natives + jar embedded). Verified: upright zero-change (paragraphs.md md5 + JSON byte-identical), italic overhang eliminated (per-glyph shift 4→22px = Σ K·(baseY−y_top), CJK 5px = 0.25×em exact), line-start/end ink unchanged.
  - **Known limits (registered)**: (1) paragraph-box max_x not extended (residual ≈2px — review suggested re-verification with layout dump, not blocking); (2) compensation assumes LTR visual order + shifts center/right/justified alignment lines (inherent to formula, mirrors engine shear assumption); (3) decoration rect extrapolation pre-compensation — currently unconstructible (italic+decoration same span has no parse path), latent; (4) pre-existing breaks+spans span_index collision (measure.rs:263-280 vs layout.rs:763-781) — pre-existing, no fixture triggers, separate issue.
