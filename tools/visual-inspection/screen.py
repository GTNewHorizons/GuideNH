...
【该文档系统的预期元素，不要报告】
- 尖括号标签（如 <GameScene>、<Entity>）以等宽代码样式出现在正文中是正常排版。
- 纯黑色遮挡条（可含少量噪点）是 Spoiler 遮罩的预期渲染。
- 页面顶部 TEST GOAL / INVARIANTS 双语说明块是测试标注，不是内容缺陷。

【判定基准】页面中灰色小字 "Expected: ..." 是对其下方内容预期效果的官方描述。将它作为对照基准：实际渲染与 Expected 描述不符才报告；相符则不报告。没有 Expected 的段落按通用排版标准判断。不要因为 Expected 文本本身的存在或措辞而报告。

请检查以下问题类别：
1. 文字重叠 - 不同文本块互相重叠，或图标/图片压在文字上
2. 文字溢出容器 - 文本超出其背景容器边界
3. 元素错位 - 元素未对齐或位置异常（图标偏离其所在行、压到相邻行或区块外也属于此类）
4. 异常空白 - 不应有的空白区域
5. 图像或3D场景破损 - 图片/3D渲染显示异常(花屏/黑块/缺失)
6. 字形渲染异常 - 文字显示不全、乱码或字形错误
7. 裁剪截断 - 内容被边缘截断
8. 字体渲染路径异常 - 同一页面内正文与图形（图表/流程图/思维导图节点、场景标注）中的文字应使用同一套平滑字体渲染；若图形内文字呈像素点阵风格、与正文平滑字体明显不一致，报告此类
9. 其他 - 其他排版缺陷

【不要报告的情况（除上述预期元素外）】
- 树形/目录树结构中，连接线（│ ├ └ 或 | |-- \--）对齐即视为缩进正确，不要凭文本语义推断父子层级关系。
- "CSV with ..."、"## ..." 等样式的行是章节标题，不是未渲染的配置文本。
- 文字排到页面或容器右缘但未被截半的正常排版；只有文字明显截半才报"裁剪截断"。
