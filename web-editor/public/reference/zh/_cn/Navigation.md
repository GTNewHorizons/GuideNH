...
| --- | --- |
| `title` | 必填，显示标题 |
| `keyword` | 可选，单个搜索关键词或别名 |
| `keywords` | 可选，搜索关键词或别名列表 |
| `parent` | 可选，父页面 id；解析规则与指南页面链接相同 |
| `position` | 可选，同级排序提示 |
| `recommend` | 可选，首页推荐优先级；缺省时不会出现在推荐面板中 |
| `priority` | 可选，同路径页面覆盖时的加载优先级；默认 `0` |
| `icon` | 可选，单个物品图标；支持内联 `mod:item:meta:{snbt}` 尾部 |
| `icons` | 可选，循环物品图标列表；每项可为带可选内联 `:{snbt}` 的普通物品 id，也可为 `{id, meta?, nbt?}` map |
...
`navigation.keyword` 添加一个搜索别名，`navigation.keywords` 添加多个搜索别名。两个字段可以同时使用；
值会自动去除首尾空白并忽略重复项。关键词使用与标题和正文相同的语言分析器及前缀模糊匹配，搜索结果仍显示正常的导航标题。
...
1. 如果配置了 `icon_textures`，优先使用循环纹理图标
2. 否则若 `icon_texture` 能成功加载，则使用单张纹理图标
3. 否则若 `icons` 中至少有一个物品可成功解析，则使用循环物品图标
4. 否则若 `icon` 对应的物品存在，则使用单个物品图标
5. 若都不可用，则不显示图标
