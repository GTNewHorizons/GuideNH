...
当 StructureLib 加载时，GuideNH 可以通过 `<ImportStructureLib>` 把多方块预览导入 `<GameScene>`。客户端指令 `/exportStructure structureLib` 也可以把这些预览导出为 PNG 文档截图。完整指令参数、StructureLib 专属选项以及相关的 `gameScene` 导出模式见 [结构导出](Structure-Export)。
...
属性表与示例参见 [标签参考](Tags-Reference#questlink)。
