...
GuideNH Web Editor is a small Node.js service and browser workspace for editing and previewing GuideNH pages. It accepts a directory selected through the browser directory picker, including a GuideNH repository such as `E:\\Github\\GuideNH-NH`, an ExportSite output directory, or a ZIP archive.
...
- ExportSite HTML pages are previewed in a sandboxed frame with `_site/app.css`, `model-viewer.css`, local fonts, textures, images, and the site search runtime inlined when available.
- ExportSite module previews recursively inline the site app, search, scene viewer, model viewer, vendor modules, and local binary scene assets. WebGL or asset failures keep the exported placeholder visible instead of breaking the page.
